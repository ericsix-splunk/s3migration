...
options=("backup to s3 bucket" "restore to s3 bucket")
...
function s3backup {
    ...
    $S3CMD sync --rexclude="(_internaldb|persistentstorage|fishbucket|sos|history|sos_summary_daily|splunklogger|summary|_introspectiondb|_introspection)\/.*" --rexclude="\/db\/hot.*" --rexclude="\/datamodel_summary\/.*" $SplunkDBLocation s3://$S3BucketLocation | tee -a $LogFile
...
function createLog {
   ...
   echo "Bucket Copy to S3 Log" >> $LogFile
   echo " " >> $LogFile
}
